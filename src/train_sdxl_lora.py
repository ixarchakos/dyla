...
def parse_args():
    ...
    parser.add_argument("--pretrained_model_name_or_path", type=str, default="stabilityai/stable-diffusion-xl-base-1.0", required=False)
    ...
    parser.add_argument("--train_sdxl", action="store_true")
...
def train_sdxl():
        ...
        if accelerator.is_main_process:
            ...
            for model in models:
                if isinstance(unwrap_model(model), type(unwrap_model(unet))):
                    ...
                    raise ValueError(f"unexpected save checkpoints: {model.__class__}")

                # make sure to pop weight so that corresponding checkpoints is not saved again
                if weights:
                    weights.pop()
    ...
    def load_model_hook(models, input_dir):
        ...
        if incompatible_keys is not None:
            ...
            if unexpected_keys:
                logger.warning(
                    f"Loading adapter weights from state_dict led to unexpected keys not found in the checkpoints: "
                    f" {unexpected_keys}. "
                )
    ...
    # Use 8-bit Adam for lower memory usage or to fine-tune the checkpoints in 16GB GPUs
    ...
    # Always use the CSV at ioannis_images/prompts.csv
    ...
    def preprocess_train(examples):
        ...
        if args.debug_loss:
            fnames = [os.path.basename(img_path) for img_path in examples[image_column]]
            if fnames:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # Add noise to the checkpoints input according to the noise magnitude at each timestep
